...
export function PptToPdfClient() {
  ...
  const handleConvert = async () => {
    if (files.length === 0) { setError('Please select a PowerPoint file.'); return }
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6">
      <FileDropzone
        ...
        label="Drag & drop your PowerPoint file here"
        sublabel="Supports .pptx and .ppt — max 50MB"
      />
      ...
      {result && <ToolResult blob={result} filename="converted.pdf" label="PowerPoint converted to PDF!" />}
    </div>
  )
}
