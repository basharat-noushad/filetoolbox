...
export function PdfToExcelClient() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      if (res.status === 422) throw new Error('No table data found in this PDF. Try a PDF with tables.')
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6">
      <FileDropzone
        ...
        sublabel="Best results with PDFs containing tables — max 50MB"
      />
      ...
      {result && <ToolResult blob={result} filename="converted.xlsx" label="PDF converted to Excel!" />}
    </div>
  )
}
