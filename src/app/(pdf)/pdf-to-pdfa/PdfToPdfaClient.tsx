...
export function PdfToPdfaClient() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/pdf-to-pdfa', { method: 'POST', body: formData })
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6">
      <FileDropzone
        ...
        sublabel="Converts to PDF/A-2b archival format — max 50MB"
      />
      ...
      {result && <ToolResult blob={result} filename="converted-pdfa.pdf" label="PDF converted to PDF/A!" />}
    </div>
  )
}
